...
public class Controller_Register extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (submit == null) {
            ...
            request.setAttribute("lsQ", lsQ);
            request.setAttribute("errorE", "Email không hợp lệ!");
            email = request.getParameter("email");
            password = request.getParameter("pass");
            repassword = request.getParameter("repass");
            fullname = request.getParameter("fullname");
            ...
            if (e.matcher(email).find()) {
                if (a.checkEmail(email)) {
                    request.setAttribute("alertE", "Email đã tồn tại!");
                    request.getRequestDispatcher("register.jsp").forward(request, response);
                } else {
                    if (!password.equals(repassword)) {
                        request.setAttribute("alertP", "Mật khẩu đã nhập không khớp. Thử lại!!");
                        request.getRequestDispatcher("register.jsp").forward(request, response);
                    } else {
                        ...
                        if (f.matcher(fullname).find()) {
                            ...
                            request.setAttribute("alertEmail", "Bạn cần xác thực qua gmail!");
                            request.getRequestDispatcher("register.jsp").forward(request, response);
                        } else {
                            request.setAttribute("alertF", "Tên đầy đủ phải bắt đầu bằng chữ!");
                            request.getRequestDispatcher("register.jsp").forward(request, response);
                        }
                    }
                }
            } else {
                request.setAttribute("alertE", "Email không hợp lệ!");
                request.getRequestDispatcher("register.jsp").forward(request, response);
            }
        }
